...
def town(id):
    ...
    try:
        res = requests.get(
            f"http://api.openweathermap.org/data/2.5/weather?q={town.name.strip()}&units=metric&type=like&appid=9f4c1fe0ad890b08c2e63e3975469f42"
        ).json()
    ...
    if request.method == "POST":
        ...
        if current_user.is_authenticated:
            comments = (
                Comments.query.filter_by(town=id)
...
def historical_places():
    ...
    return render_template(
        "historical_places.html", historical_places=historical_places
    )
